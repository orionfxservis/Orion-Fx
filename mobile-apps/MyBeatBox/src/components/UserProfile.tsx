  ...
  return (
    <div className="w-full max-w-md mx-auto flex flex-col items-center">
      ...
      <div className="w-full bg-[#0d121f]/95 border border-white/10 rounded-2xl p-6 sm:p-7 shadow-2xl backdrop-blur-xl flex flex-col">
        ...
        {/* Guest Sign-In CTA — shown when not authenticated */}
        {!isAuthenticated && (
          <div className="flex flex-col items-center text-center gap-4 py-6">
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-white/[0.06] border-2 border-dashed border-white/20 flex items-center justify-center">
              <User className="w-9 h-9 text-white/30" />
            </div>
            <div className="flex flex-col gap-1">
              <h2 className="text-lg sm:text-xl font-bold text-white tracking-tight">Guest</h2>
              <p className="text-xs text-white/50 font-medium">Sign in to access your profile</p>
            </div>
            {onSignIn && (
              <button
                onClick={onSignIn}
                className="flex items-center gap-2.5 px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500/20 to-blue-500/20 hover:from-cyan-500/30 hover:to-blue-500/30 border border-cyan-500/30 hover:border-cyan-400/50 text-cyan-300 hover:text-cyan-200 text-sm font-semibold transition-all duration-200 active:scale-95 cursor-pointer shadow-lg shadow-cyan-500/10"
                id="btn-profile-google-sign-in"
              >
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="text-white/50 hover:text-white/80 text-xs font-medium mt-2 transition cursor-pointer"
              >
                Close
              </button>
            )}
